(ns project_tracker.common.result)

(defn- deep-merge-with
  "Like merge-with, but merges maps recurcively, applying the given fn
...
(defn add-error
  ([value key msg]
     (add-error value {:error {key [msg]}}))
  ([value errors]
     (deep-merge-with (fn [& msgs] (vec (flatten (concat msgs))))
                      (result value)
                      (select-keys errors [:errors]))))
...
(defmacro merge-error->
  "Threads the results through the forms while merging the errors.
   Inserts the result value as the second item in the first form,
   making a list of it, if it is not a list already. If there are more forms,
...
(defmacro until-error->
  "Threads the results through the forms until the first error.
   Inserts the result value as the second item int the first form, making a list of
   it, if it is not a list already. If there more forms, inserts the first
  ...
  ([x form & more] '(unil-error-> (until-error-> ~x ~form) ~@more)))