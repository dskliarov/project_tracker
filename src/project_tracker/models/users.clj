...
(defn user [query]
  ...
  (defn username-free? [{:keys [username] :as user}]
    (if (= 0 (mc/count collection {:username username}))
      (result/result user)
      (result/error user :name "The name alreasy exists")))
  ...
  (defn- update-raw! [user]
    (try
      (do
        (let [update-reuslt (mc/update-by-id collection (:_id user) user)]
  ...
  (defn update-valid? [new-user current-user]
    (if (or (= (:username new-user) (:username current-user)) (not (result/has-error? (username-free? new-user))))
      ...
      (result/error new-user :name "Username already in use.")))
  ...
  (defn update! [user]
    (result/until-error-> user
                          ...
                          (update-raw!)))
  (defn remove! [id]
    (result/result (mc/remove-by_id collection id)))
